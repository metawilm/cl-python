...
(defun walk-py-ast (ast f &key (walk-lists-only t))
  ...
  "Walk recursively through AST, calling F on each statement. During the
call, *py-ast-lvalue* will have value T iff the form is an l-value
(assignment target).
...
(defun ast-recurse-fun (ast)
  ...
  (ecase (car ast)
    ...
    (assert         (lambda (f form &key value target)
		      ...
		      `(assert ,(funcall f (second form) t nil)
