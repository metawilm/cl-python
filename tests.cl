...
;; Please note that these tests are based on a different AST than what
;; now gets generated by the parser. In other words, some of the tests
;; below are outdated.
  ...
  `(progn
     ,@(loop for (eqlfun . cases) in suite
	   append (loop for (what expected) in cases
		      collect
			`(progn #+(or)(format t "trying: ~S ...~%" ',what)
				#+(or)(format t "  result: ~S   expected: ~S~%"
					      ,what ',expected)
...
(parser-test
 (("0" 0)
  ...
  ("x[0]" (subscription (identifier x) ((0) nil)))
  ("x[0,1]" (subscription (identifier x) ((0 1) nil)))
  ("x(0)" (call (identifier x) (0)))
  ("x(0,1)" (call (identifier x) (0 1)))
  ("x(*args)" (call (identifier x) ((* (identifier args)))))
  ("x(**kw)" (call (identifier x) ((** (identifier kw)))))
  ("x(*args, **kwargs)" (call (identifier x) ((* (identifier args)) (** (identifier kwargs)))))
  ("x(1,a=5)" (call (identifier x) (1 (= (identifier a) 5))))
  ("f(a,*a, **w)" (call (identifier f) ((identifier a) (* (identifier a)) (** (identifier w)))))
...
(assert-all
 ...
 (py-== (make-tuple 1 2 3)
	(user-py-eval '(testlist (1 2 3) nil)))
